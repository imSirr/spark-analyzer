...
(function () {
  ...
  async function refreshLatestMc() {
    // Mojang version manifest is CORS-enabled; best-effort only.
    try {
      const r = await fetch('https://launchermeta.mojang.com/mc/game/version_manifest_v2.json');
      if (r.ok) { const j = await r.json(); if (j && j.latest && j.latest.release) SparkRules.setLatestMc(j.latest.release); }
    } catch (e) { /* ignore — fallback constant is used */ }
  }
  ...
  function renderSystem(ctx) {
    ...
    if (sys.gc && Object.keys(sys.gc).length) {
      ...
      Object.entries(sys.gc).forEach(([name, g]) => tb.appendChild(el('tr', {}, [
        td(name), td(Number(g.avgTime).toFixed(1) + ' ms'), td('every ' + Number(g.avgFrequency).toFixed(1) + ' s'), td(fmtNum(g.total)),
      ])));
